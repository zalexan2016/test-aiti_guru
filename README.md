...
Интерактивная документация API (генерируется автоматически FastAPI):
...
После первого запуска можно заполнить БД фейковыми данными (категории из ТЗ, товары, клиенты, заказы):
